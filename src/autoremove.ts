...
export class AutoRemove {
    ...
    check(torrents: {[key: string]: Torrent}) {
        ...
        for(const[key, torrent] of Object.entries(torrents)) {
            ...
            let removeRules = AutoRemove.getRemoveRules(torrent);
            let appliedFilterResults = AutoRemove.applyFilters(removeRules, torrent);
            let shouldRemove = appliedFilterResults.length === 1 ? appliedFilterResults[0] : conjunctions[removeRules.conjunction](appliedFilterResults);
    ...
    private static isExempted(torrent) {
        return config.autoremove.exemptions && (config.autoremove.exemptions[torrent.label] || config.autoremove.exemptions[torrent.tracker_host]);
    }
